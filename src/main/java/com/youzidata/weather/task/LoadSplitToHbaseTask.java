...
public class LoadSplitToHbaseTask {
     ...
     * @return
     */
    public Map<String,Object> split(List<String> fileList, double step_length, int thread_num) {
...
//                admin.createTable(hbaseTable);
                    ...
                    public void run() {
                        try {
                            ...
                            try {
                                ...
                                res = parseFile(date, openNC, nanMap, writeMap, readMap, step_length, thread_num);
     ...
     * @return
     */
    public Map<String,Object> splitString(List<String> fileList, double step_length, int thread_num) {
        long startTime = System.currentTimeMillis();
...
//        String str = fileList.get(0).substring(fileList.get(0).length()-15, fileList.get(0).length()-3);
        ...
        try {
            conn = ConnectionFactory.createConnection(hbaseTemplate.getConfiguration());
            admin = conn.getAdmin();
            if (!admin.isTableAvailable(TableName.valueOf("weather_"+date+"_old"))) {
                HTableDescriptor hbaseTable = new HTableDescriptor(TableName.valueOf("weather_"+date+"_old"));
                hbaseTable.addFamily(new HColumnDescriptor("cf"));
        ...
        //开始解析nc文件并导入Hbase
        ExecutorService executor = Executors.newFixedThreadPool(2);
        Map<String, Long> writeMap = new HashMap<>();//写入时间
        Map<String, Long> nanMap = new HashMap<>();//解析时间
        Map<String, Long> readMap = new HashMap<>();//读取时间
        List<String> errorPath = new ArrayList<>();
        try {
            for(String path :fileList ) {
                executor.execute(new Runnable() {//向线程池中添加线程
                    ...
                    public void run() {
                        try {
                            ...
                            try {
                                ...
                                System.out.println("-----打开文件用时:"+(endTime - startTime));
                                String date = path.substring(path.length()-15, path.length()-5);
                                res = parseFileToString(date, openNC, nanMap, writeMap, readMap, step_length, thread_num);
                                long end = System.currentTimeMillis();
                                System.out.println("-----解析加保存用时："+(end - endTime));
                            } catch (IOException e) {
                                System.out.println("parseFile()");
     ...
     * @return
     */
    public  boolean parseFile(String date,NetcdfFile openNC,Map<String, Long> nanMap,Map<String, Long> wirteMap,Map<String, Long> readMap, double step_length, int thread_num){
        boolean res = true;

        List<Put> puts = new ArrayList<>();//一个rowkey中的，一个列，一个put对象
//        Connection conn = null;
        long analysisStartTime = System.currentTimeMillis();//解析开始时间
        try {
//            conn = ConnectionFactory.createConnection(hbaseTemplate.getConfiguration());
...
//            int split_interval = 1000;//表示每1000个单位切分一次
            ...
            int split_interval = (int)(10.0/step_length);
...
//            ExecutorService executor = Executors.newFixedThreadPool(18);
            for(int a = 0; a < split_num; a++) {
                for(int b = 0; b < split_num; b++) {
                    ...
                    Map<String,Future<byte[]>> twoMap = new HashMap<>();//存储二维的属性，用于多线程计算矩阵的参数
                    ...
                    for (Variable variable : variables) {
                        if (variable.getDimensions().size() == 3) {//一个属性有三个维度的情况
                            ...
                            ParseTwoFloatThread thread = new ParseTwoFloatThread(resultData,lonstr,lat);
                            twoMap.put(variable.getShortName(),service.submit(thread));
                        }
                    }
                    readMap.put(a+""+b+"read", System.currentTimeMillis() - analysisStartTime);
                    ...
                    //二维属性的rowkey生成规则
                    String rowkey = date + "_" + 0 + "_" + (int)latArray[latArray.length-1] + "_" + (int)latArray[0] + "_" + (int)lonArray[0] + "_" + (int)lonArray[lonArray.length - 1] + "_" + step_length;
                    System.out.println(rowkey);
                    rowkey_list.add(rowkey);
                    Put put = new Put(Bytes.toBytes(rowkey));
                    for(Map.Entry<String,Future<byte[]>> entry : twoMap.entrySet()){
                        put.addColumn(family, Bytes.toBytes(entry.getKey()), entry.getValue().get());
                    }
                    puts.add(put);
            ...
            hbaseDao.multithreadingInsert("weather_"+date, puts, thread_num);
     ...
     * @return
     */
    public  boolean parseFileToString(String date,NetcdfFile openNC,Map<String, Long> nanMap,Map<String, Long> wirteMap,Map<String, Long> readMap, double step_length, int thread_num){
        boolean res = true;
        long analysisStartTime = System.currentTimeMillis();//解析开始时间
        List<Put> puts = new ArrayList<>();//一个rowkey中的，一个列，一个put对象
//        Connection conn = null;
...
//            ExecutorService executor = Executors.newFixedThreadPool(18);
            for(int a = 0; a < split_num; a++) {
                for(int b = 0; b < split_num; b++) {
                    ...
                    for(Map.Entry<String,Future<String>> entry : twoMap.entrySet()){
                        put.addColumn(family, Bytes.toBytes(entry.getKey()), Bytes.toBytes(entry.getValue().get()));
                    }
                    puts.add(put);
            ...
            //向Hbase插入数据
            long writeStartTime = System.currentTimeMillis();//写的开始时间
            hbaseDao.multithreadingInsert("weather_"+date+"_old", puts, thread_num);
            long writeEndTime = System.currentTimeMillis();
            wirteMap.put("write", writeEndTime - writeStartTime);
        } catch (Exception ex){
            ex.printStackTrace();
...
//                date+"_0_"+"30.0_40.0",date+"_0_"+"20.0_30.0",date+"_0_"+"10.0_20.0",date+"_0_"+"0.0_10.0",date+"_0_"+"-10.0_0.0",
...
//                date+"_0_"+"-70.0_-60.0",date+"_0_"+"-80.0_-70.0",date+"_0_"+"-90.0_-80.0"};
        String[] keys = new String[] { date+"_0_"+"80_90", date+"_0_"+"70_80", date+"_0_"+"60_70", date+"_0_"+"50_60", date+"_0_"+"40_50",
                date+"_0_"+"30_40",date+"_0_"+"20_30",date+"_0_"+"10_20",date+"_0_"+"0_10",date+"_0_"+"-10_0",
                date+"_0_"+"-20_-10",date+"_0_"+"-30_-20",date+"_0_"+"-40_-30",date+"_0_"+"-50_-40",date+"_0_"+"-60_-50",
                date+"_0_"+"-70_-60",date+"_0_"+"-80_-70",date+"_0_"+"-90_-80"};
