...
import com.youzidata.weather.task.ParseThreeThread;
import com.youzidata.weather.task.ParseTwoThread;
 ...
 */
@Service
public class LoadSingleElementFileService {
...
//        String[] keys = new String[] { date+"0_", date+"30_", date+"60_", date+"90_", date+"120_",date+"150_",date+"180_",date+"210_",date+"240_"};
    ...
    /**
     * 获取给定目录中，符合时间范围的所有nc文件，并且将这些文件入Hbase
     * @param dirPath
     * @param startDate
     * @param endDate
     * @return
...
//        FileUtil.filelist = new ArrayList<>();//将储存文件路径的静态变量初始化
//        FileUtil.getFileList(dirPath);//所有nc文件的File对象都存储在FileUtil中的静态变量中
//        List<File> filelist = FileUtil.getFilelist2(dirPath);//当前目录下的所有nc文件File对象
...
//        }
    ...
    /**
     * 传入需要存入Hbase的文件集合，并且将这些文件入Hbase
     * @return
     * @throws ParseException
     */
    public Map<String, Object> test(List<File> fileList) throws ParseException {
        Long startTime = System.currentTimeMillis();//入库开始时间
        System.out.println("-----"+"开始执行："+startTime);
        ...
        Map<String, Object> resultMap = loadList(fileStrList);

        long endTime = System.currentTimeMillis();
        System.out.println("-----"+"程序结束:"+endTime);
        System.out.println("-----"+"总共入库用时："+(endTime-startTime));
        resultMap.put("sumTime", endTime-startTime);
        return  resultMap;
    }
...
//            String userId = StringUtil.getUserIdFromNc(file_temp.getName());
            ...
            res = parseFile(path,openNC,anaMap,writeMap,readMap);
...
//	}
     ...
     * @throws
     */
    public  boolean parseFile(String path, NetcdfFile openNC,Map<String, Long> nanMap,Map<String, Long> wirteMap,Map<String, Long> readMap){
        File file_temp = new File(path);
        String fileName = file_temp.getName();
...
//            table.setAutoFlush(false);
            ...
            try{
                for(int i = 0; i < lvThreeLength; i++) {
                    String rowkey = date + timeRange + "_" + (i+1);
                    Put put = new Put(Bytes.toBytes(rowkey));
                    for(Map.Entry<String,Future<List<String>>> entry : threeMap.entrySet()){
...
//                }
                //如果是二维属性，没有时效参数，那么写0
                String rowkey = date + timeRange + "_" + 0;
                ...
                //入库成功的文件路径会记录下来
                TxtUtil.writeFile(txtPath + File.separator + StringUtil.getDateStrFromNc(fileName, 8)+ ".log", path);
            }catch(Exception e){
                e.printStackTrace();
            }finally {
            }
